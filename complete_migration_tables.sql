...
insert into public.shipping_status (
	...
	with fin as (
		select
    		...
    		max(state_datetime) AS shipping_fact_datetime
		from 
    		public.shipping
		group by shippingid),
	fin_sh as (
		select 
			...
			, t1.status::varchar as status
			, t1.state::varchar as state
			, t.shipping_start_fact_datetime
			, t.shipping_end_fact_datetime
		from fin t
		join public.shipping t1 on t.shippingid = t1.shippingid
			and t1.state_datetime = (case when t.shipping_end_fact_datetime isnull then t.shipping_fact_datetime 
				else t.shipping_end_fact_datetime end)
		)
